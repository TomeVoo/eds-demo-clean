function renderDetails(data) {
  ...
  html += `<h3>Decyzja systemu EDS</h3>`;

  const exams = data.examinations || [];

  if (exams.length === 0) {
    html += `<p>Brak badań dla tego narażenia.</p>`;
  } else {
    ...
    exams.forEach(ex => {
      ...
      if (currentAge !== null) {
        if (ex.min_age !== null && currentAge < ex.min_age) {
          applicable = false;
          reason = `dotyczy od ${ex.min_age} lat`;
        }
        if (ex.max_age !== null && currentAge > ex.max_age) {
          applicable = false;
          reason = `dotyczy do ${ex.max_age} lat`;
        }
      }

      if (applicable) {
        html += `
          <li style="margin-bottom:14px; padding:10px; border-left:4px solid #1f3a5f; background:#f8f9fb;">
            <strong>${ex.name}</strong>
            <span style="margin-left:6px; color:${ex.required ? "#c0392b" : "#555"}">
              (${ex.required ? "WYMAGANE" : "OPCJONALNE"})
            </span>

            <div style="font-size:13px; color:#333; margin-top:6px;">
              ${ex.min_age !== null ? `• od ${ex.min_age} roku życia<br>` : ""}
        ...
        html += `
          <li style="margin-bottom:10px; padding:8px; color:#999;">
